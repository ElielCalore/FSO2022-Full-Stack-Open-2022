    ...
    async function fetchWeather(singleCity) {
      try {
        const response = await axios.get(
          `http://api.weatherapi.com/v1/current.json?key=${API_KEY}&q=${singleCity}&aqi=no`
        );
    ...
    return (
      <>
        ...
        <ul>
          {objKeys.map((current) => {
            return (
              <li key={current}>{countriesSearch[0].languages[current]}</li>
            );
          })}
        </ul>

        <img
          src={countriesSearch[0].flags.png}
          alt="flags"
          width="200"
          height="150"
        />
